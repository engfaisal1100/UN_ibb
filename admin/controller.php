...
function updateAbout($description, $vision, $mission, $established, $goals) {
    try {
        // تحميل البيانات من ملف JSON
        $data = loadData();

        // تنظيف الأهداف: إزالة العناصر الفارغة
        $cleanGoals = array_filter($goals, fn($goal) => trim($goal) !== '');

        // تحديث بيانات "عن الجامعة"
        $data['about']['description'] = $description;
        $data['about']['vision'] = $vision;
        $data['about']['mission'] = $mission;
        $data['about']['established'] = $established;
        $data['about']['goals'] = array_values($cleanGoals); // إعادة فهرسة الأهداف
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // تمرير جميع الحقول المطلوبة لدالة updateAbout
    updateAbout(
        $_POST['description'],
        $_POST['vision'],
        $_POST['mission'],
